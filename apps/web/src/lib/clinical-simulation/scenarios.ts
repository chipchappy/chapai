...
const postoperativeBleeding: ClinicalScenarioInput = {
  ...
  prebrief: {
    ...
    handoff: "Maria Gonzalez returned from open colectomy two hours ago. PACU reported stable vital signs, a patent peripheral IV, Foley catheter, and a Jackson-Pratt drain with 45 mL sanguineous output at transfer.",
  ...
  patient: {
    name: "Maria Gonzalez", age: 64, sex: "female", pronouns: "she/her", room: "4B-412",
  ...
  actions: [
    ...
    action("notify-surgery", "Notify surgical provider using SBAR", "communication", "essential", "Urgent, complete communication should include the trend, drain output, perfusion findings, actions taken, and a specific request for bedside evaluation.", [postopSource.id], {
      prerequisites: ["repeat-vitals", "assess-drain"],
      lateAfterMinute: 8,
      communication: {
        prompt: "Select the elements to include in the urgent surgical SBAR.",
        elements: sbarElements,
        requiredElementIds: ["identity", "concern", "assessment", "interventions", "request"],
        // CLINICAL REVIEW: Exact hemorrhage protocols and transfusion thresholds vary by facility and provider judgment.
        responses: [
          { condition: { notCompleted: ["assess-perfusion"] }, message: "The surgical provider asks for the current MAP, mental status, urine output, capillary refill, and IV status before finalizing the rescue plan.", effects: [], delayMinutes: 0 },
          { condition: { all: [{ path: "vitals.map", operator: "lt", value: 65 }] }, message: "The surgical provider activates the hemorrhage response, orders urgent coagulation and type-and-cross testing, directs the team to hold anticoagulation, and prepares blood-product and source-control pathways.", effects: [push("activeOrders", "Urgent CBC, coagulation studies, fibrinogen, and type-and-cross"), push("activeOrders", "Hold anticoagulants pending surgical reassessment"), push("activeOrders", "Prepare blood products and urgent source-control evaluation"), set("flags.hemorrhageResponseActivated", true)], delayMinutes: 1 },
          { message: "The surgical provider orders urgent CBC, coagulation studies, and type-and-cross testing, directs the team to hold anticoagulation, and comes for bedside reassessment.", effects: [push("activeOrders", "Urgent CBC, coagulation studies, and type-and-cross"), push("activeOrders", "Hold anticoagulants pending surgical reassessment"), set("flags.hemorrhageResponseActivated", true)], delayMinutes: 1 },
        ],
      },
    ...
    action("wait-repeat-hemoglobin", "Wait for repeat hemoglobin before escalating", "intervention", "low_value", "An acute hemoglobin value can lag behind active blood loss; the worsening hemodynamic, perfusion, and drain-output trend requires escalation without waiting for laboratory confirmation.", [postopSource.id], { effects: [set("flags.waitedForHemoglobin", true)], feedback: "Waiting for another hemoglobin delays rescue despite converging hemorrhage cues." }),
    action("reinforce-dressing-routine-recheck", "Reinforce the dressing and recheck at the routine interval", "intervention", "low_value", "Covering drainage without promptly measuring the trend, assessing perfusion, and escalating does not address suspected postoperative hemorrhage.", [postopSource.id], { effects: [set("flags.dressingCoveredWithoutEscalation", true)], feedback: "The dressing is covered, but the active bleeding and perfusion trend remain unaddressed." }),
...
const acuteCoronarySyndrome: ClinicalScenarioInput = {
  ...
  actions: [
    ...
    action("notify-cardiology", "Notify cardiology using focused SBAR", "communication", "essential", "Dynamic symptoms and ECG changes require urgent communication with a specific request for bedside evaluation and escalation.", [acsSource.id], {
      prerequisites: ["assess-chest-pain", "repeat-ecg"],
      lateAfterMinute: 7,
      communication: {
        prompt: "Build the urgent cardiac SBAR.",
        elements: sbarElements,
        requiredElementIds: ["identity", "concern", "assessment", "background", "request"],
        // CLINICAL REVIEW: Local ACS pathways determine exact serial-test intervals and medication orders.
        responses: [
          { condition: { notCompleted: ["review-acs-safety"] }, message: "Cardiology asks the nurse to confirm allergy, bleeding risk, blood pressure, and recent phosphodiesterase-5 inhibitor use before medication decisions.", effects: [], delayMinutes: 0 },
          { condition: { all: [{ path: "flags.tadalafilRecent", operator: "eq", value: true }] }, message: "Cardiology activates the ACS pathway, orders continuous telemetry with serial ECG and troponin testing, confirms that nitrates remain withheld, and supports aspirin when the safety review is clear.", effects: [push("activeOrders", "Continuous telemetry and serial 12-lead ECGs"), push("activeOrders", "Serial cardiac troponin testing per ACS pathway"), push("activeOrders", "Withhold nitrates because of recent tadalafil exposure"), set("flags.acsPathwayActivated", true)], delayMinutes: 1 },
          { message: "Cardiology activates the ACS pathway and orders continuous telemetry with serial ECG and troponin testing while arranging urgent bedside evaluation.", effects: [push("activeOrders", "Continuous telemetry and serial 12-lead ECGs"), push("activeOrders", "Serial cardiac troponin testing per ACS pathway"), set("flags.acsPathwayActivated", true)], delayMinutes: 1 },
        ],
      },
      effects: [set("flags.cardiologyNotified", true)],
      score: [score("communication", 4), score("escalation", 4)],
    }),
    ...
    action("wait-for-troponin", "Wait for troponin before notifying cardiology", "intervention", "low_value", "Persistent ischemic symptoms with dynamic ECG change require urgent escalation; a pending biomarker should not delay response to the current high-risk presentation.", [acsSource.id], { effects: [set("flags.waitedForTroponin", true)], feedback: "Cardiology notification is delayed while clinically actionable symptoms and ECG changes are already present." }),
    action("treat-as-reflux", "Treat the discomfort as reflux before repeating the ECG", "intervention", "low_value", "Assuming a benign gastrointestinal cause before evaluating recurrent pressure, diaphoresis, nausea, and dynamic ischemic change can delay time-sensitive ACS care.", [acsSource.id], { effects: [set("flags.refluxAssumption", true)], feedback: "A benign explanation is pursued before the evolving cardiac presentation is reassessed." }),
...
const respiratoryDeterioration: ClinicalScenarioInput = {
  ...
  actions: [
    ...
    action("titrate-oxygen", "Titrate oxygen within ordered target", "intervention", "essential", "Hypoxemia requires controlled oxygen titration to the ordered range with close assessment rather than withholding oxygen or using an unmonitored fixed approach.", [copdSource.id], { prerequisites: ["assess-respiratory"], effects: [set("oxygenDevice", "Venturi mask"), set("oxygenFlow", "controlled concentration titrated to 88%-92%"), set("vitals.spo2", 90)], score: [score("intervention", 4), score("safety", 3)] }),
    ...
    action("contact-rt", "Contact respiratory therapy for NIV", "communication", "essential", "Acidemic hypercapnic respiratory failure with distress requires prompt team evaluation for noninvasive ventilatory support.", [copdSource.id], {
      prerequisites: ["review-blood-gas"],
      communication: {
        prompt: "Select the information to communicate to respiratory therapy.",
        elements: sbarElements,
        requiredElementIds: ["identity", "concern", "assessment", "interventions", "request"],
        // CLINICAL REVIEW: NIV settings, repeat-gas timing, and intubation thresholds require local protocol and bedside judgment.
        responses: [
          { condition: { notCompleted: ["assess-neuro"] }, message: "Respiratory therapy asks the nurse to confirm alertness, secretion burden, and airway-protection ability before beginning noninvasive ventilation.", effects: [push("activeOrders", "Prepare NIV equipment while confirming airway-protection suitability")], delayMinutes: 0 },
          { condition: { all: [{ path: "labs.pH", operator: "lte", value: 7.3 }] }, message: "Respiratory therapy begins a monitored bilevel NIV trial targeting oxygen saturation of 88%-92%, requests a repeat blood gas, and establishes explicit escalation criteria for worsening mentation, gas exchange, or fatigue.", effects: [push("activeOrders", "Monitored bilevel NIV trial; target SpO2 88%-92%"), push("activeOrders", "Repeat blood gas after NIV initiation"), push("activeOrders", "Escalate for invasive airway evaluation if NIV fails"), set("flags.nivPlanActive", true)], delayMinutes: 1 },
          { message: "Respiratory therapy recommends a monitored NIV trial with controlled oxygen, bronchodilator therapy, and serial respiratory and mental-status reassessment.", effects: [push("activeOrders", "Monitored NIV trial with controlled oxygen and serial reassessment"), set("flags.nivPlanActive", true)], delayMinutes: 1 },
        ],
      },
      effects: [set("flags.rtAtBedside", true)],
      score: [score("communication", 4), score("escalation", 3)],
    }),
    action("start-niv", "Initiate ordered noninvasive ventilation", "intervention", "essential", "NIV can improve gas exchange and reduce work of breathing in appropriate hypercapnic failure, with close monitoring for failure.", [copdSource.id], { prerequisites: ["contact-rt"], safetyChecks: ["assess-neuro"], effects: [set("flags.nivStarted", true), set("oxygenDevice", "bilevel noninvasive ventilation"), set("vitals.spo2", 92), add("vitals.respiratoryRate", -5)], delayedEffects: [{ delayMinutes: 5, effects: [add("labs.pCO2", -8), add("labs.pH", 0.04), set("levelOfConsciousness", "alert and less fatigued")], feedback: "Gas exchange and mental status improve after NIV initiation." }], score: [score("intervention", 5), score("prioritization", 3)] }),
    ...
    action("pursed-lip-only", "Coach pursed-lip breathing as the only intervention", "intervention", "low_value", "Pursed-lip breathing may support comfort, but it does not replace controlled oxygen, bronchodilation, blood-gas review, ventilatory support, and escalation in acute hypercapnic respiratory failure.", [copdSource.id], { effects: [set("flags.pursedLipBreathingOnly", true)], feedback: "The coaching is supportive, but hypoxemia and ventilatory failure remain untreated." }),
    action("sedate-for-niv", "Administer a sedative to improve mask tolerance before reassessment", "medication", "unsafe", "Sedation in a somnolent patient with hypercapnic ventilatory failure can further depress ventilation and airway protection; tolerance problems require reassessment and team-guided management.", [copdSource.id], { effects: [add("vitals.respiratoryRate", -3), set("levelOfConsciousness", "increasingly somnolent")], feedback: "Respiratory drive and alertness worsen after an unverified sedating approach." }),
...
const sedationCompromise: ClinicalScenarioInput = {
  ...
  prebrief: { shift: "Outpatient procedural suite", role: "RN monitoring a patient during moderate sedation", resources: ["Procedural physician", "Airway cart", "Anesthesia response", "Reversal medications under order"], handoff: "William Thompson is scheduled for endoscopy with moderate sedation. Consent is signed, IV access is present, and fentanyl plus midazolam are ordered in incremental doses. You are responsible for continuous monitoring and recovery documentation.", safetyNote: "Educational simulation only. Sedation credentials, medication authority, airway rescue, and reversal use are governed by facility policy and provider orders." },
  patient: { name: "William Thompson", age: 52, sex: "male", pronouns: "he/him", room: "Procedure 2", presentingProblem: "Elective endoscopy", history: ["Obstructive sleep apnea", "GERD"], surgicalHistory: [], psychiatricHistory: [], medicationHistory: ["Omeprazole"], allergies: ["No known drug allergies"], socialHistory: ["Spouse will accompany home"], substanceUseHistory: ["No substance use"], baselineFunction: "Independent", codeStatus: "Full code", isolation: "Standard precautions", risks: { fall: "high after sedation", suicide: "not indicated", elopement: "low", skin: "low" } },
  ...
  actions: [
    ...
    action("call-airway-response", "Activate emergency airway response", "communication", "essential", "Persistent hypoventilation requires rapid access to personnel capable of advanced airway rescue.", [sedationSource.id], {
      prerequisites: ["assess-airway"],
      communication: {
        prompt: "Select the information to give the airway response team.",
        elements: sbarElements,
        requiredElementIds: ["identity", "concern", "assessment", "request"],
        // CLINICAL REVIEW: Reversal dosing and airway-response workflows vary by active orders and local protocol.
        responses: [
          { condition: { notCompleted: ["reposition-airway"] }, message: "The response team directs immediate airway repositioning, suction readiness, and bag-mask ventilation while advanced airway equipment is brought to the bedside.", effects: [push("activeOrders", "Immediate airway maneuvers, suction readiness, and bag-mask support")], delayMinutes: 0 },
          { condition: { all: [{ path: "flags.opioidReceived", operator: "eq", value: true }] }, message: "The airway team continues ventilatory support, confirms titrated naloxone under the active reversal order, and orders prolonged monitoring for recurrent respiratory depression.", effects: [push("activeOrders", "Continue ventilatory support and titrate naloxone per active reversal order"), push("activeOrders", "Monitor for recurrent respiratory depression after reversal")], delayMinutes: 1 },
          { message: "The airway team continues assisted ventilation and prepares advanced airway rescue while the cause and depth of sedation are reassessed.", effects: [push("activeOrders", "Continue assisted ventilation and prepare advanced airway rescue")], delayMinutes: 1 },
        ],
      },
      effects: [set("flags.airwayTeamCalled", true)],
      score: [score("escalation", 4), score("communication", 2)],
    }),
    ...
    action("reassess-recovery", "Reassess airway and recovery criteria", "assessment", "essential", "Recovery requires sustained airway patency, ventilation, oxygenation, hemodynamic stability, and return toward baseline after reversal.", [sedationSource.id], { prerequisites: ["reposition-airway"], reassessmentForActionIds: ["administer-naloxone"], repeatable: true, revealFindings: ["recovery-finding"], effects: [set("flags.recoveryReassessed", true)], score: [score("reassessment", 4)] }),
    action("oxygen-only-continue-procedure", "Increase oxygen and continue the procedure without airway support", "intervention", "unsafe", "Supplemental oxygen does not correct obstruction or hypoventilation and can delay recognition if ventilation and responsiveness are not addressed immediately.", [sedationSource.id], { effects: [add("vitals.spo2", 2), add("vitals.respiratoryRate", -2)], feedback: "Saturation changes briefly, but ventilation and airway obstruction continue to worsen." }),
    action("wait-for-desaturation", "Wait for saturation to fall further before intervening", "intervention", "critical_error", "Capnography, respiratory effort, and responsiveness can show dangerous hypoventilation before profound desaturation; waiting discards the earlier rescue window.", [sedationSource.id], { effects: [add("vitals.spo2", -8), add("vitals.respiratoryRate", -3)], criticalError: "Airway rescue delayed despite clear hypoventilation" }),
...
const psychiatricSafety: ClinicalScenarioInput = {
  ...
  actions: [
    ...
    action("notify-behavioral-team", "Notify behavioral health provider and charge nurse", "communication", "essential", "The team needs the direct risk findings, current behavior, safety actions, substance history, and a request for urgent evaluation.", [suicideSource.id], {
      prerequisites: ["direct-suicide-assessment"],
      communication: {
        prompt: "Build the safety-focused SBAR.",
        elements: sbarElements,
        requiredElementIds: ["identity", "concern", "background", "assessment", "interventions", "request"],
        // CLINICAL REVIEW: Observation level, security involvement, and emergency medication require policy and patient-specific orders.
        responses: [
          { condition: { all: [{ path: "flags.environmentSecured", operator: "ne", value: true }] }, message: "The behavioral health provider directs continuous observation, immediate removal of accessible means, reduced stimulation, and urgent face-to-face evaluation.", effects: [push("activeOrders", "Continuous observation with immediate environmental safety precautions"), push("activeOrders", "Urgent behavioral health face-to-face evaluation")], delayMinutes: 0 },
          { condition: { all: [{ path: "agitation", operator: "gte", value: 8 }] }, message: "The team confirms continuous observation and least-restrictive de-escalation, with security support reserved for imminent danger while urgent evaluation proceeds.", effects: [push("activeOrders", "Continuous observation and least-restrictive de-escalation"), push("activeOrders", "Security support only for imminent danger or unsafe elopement"), set("flags.highRiskPlanActive", true)], delayMinutes: 1 },
          { message: "The team classifies the presentation as high risk, orders continuous observation and environmental safety precautions, and requests serial suicide-risk and behavior reassessment.", effects: [push("activeOrders", "Continuous observation and environmental safety precautions"), push("activeOrders", "Serial suicide-risk and behavior reassessment"), set("flags.highRiskPlanActive", true)], delayMinutes: 1 },
        ],
      },
      effects: [set("flags.teamNotified", true)],
      score: [score("communication", 4), score("escalation", 4)],
    }),
    ...
    action("promise-confidentiality", "Promise that suicide disclosures will remain confidential", "communication", "unsafe", "The nurse should explain privacy limits honestly because imminent safety concerns require team communication and protective action; promising secrecy undermines trust and delays intervention.", [suicideSource.id], { effects: [set("flags.confidentialityPromised", true)], feedback: "An unsafe promise conflicts with the need to communicate imminent risk and arrange protection." }),
