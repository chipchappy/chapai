...
|---|---|---|---|
| 2026-07-08 | `74da845d` (restored) | prod-guardian + main-deploy neuter | **INCIDENT + PREVENTION.** Rogue one-off deploy `d290aef9` (03:57) shipped the OLD main app to prod (/pricing,/study,/instructor 404; no "Study now" nav; no --c-bg) via the main checkout's `opennextjs-cloudflare deploy` + OAuth wrangler. Restored by rollback to last-good 74da845d (all features incl. instructor intact). PREVENTION: (1) `scripts/prod-guardian.ps1` scheduled task "Clarity Prod Guardian" (every 7 min) — functional health probe (/pricing 200 + Study-now nav + --c-bg); 2 consecutive broken checks → auto-rollback to stable last-good; never fires on healthy gate deploys. (2) main checkout `apps/web` deploy script neutered to a refusal. |
